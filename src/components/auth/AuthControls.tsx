...
export function AuthControls({
  ...
  return (
    <div ref={wrapRef} className={cn('relative', variant === 'block' && 'w-full')}>
      ...
      {menuRendered ? (
        // لایه‌ی بیرونی: فقط جای‌گذاری. منو از لبه‌ی چپِ تریگر «به داخلِ
        // صفحه» باز می‌شود (left-0)؛ نسخه‌ی قبلی وسط‌چین بود (left-1/2 +
        // -translate-x-1/2) و چون تریگر در هدرِ RTL به لبه‌ی چپِ ویوپورت
        // می‌چسبد، روی گوشی حدود نیمی از منو بیرونِ صفحه می‌افتاد و با
        // overflow-x: hiddenِ سراسری بریده می‌شد (گزارشِ کارفرما). سقفِ
        // max-w تضمین می‌کند حتی روی پهناترین چیدمان‌ها هم منو داخلِ
        // ویوپورت بماند. انیمیشنِ transform روی لایه‌ی درونی است تا با
        // مبدأِ جای‌گذاری تداخل نکند.
        <div className="absolute left-0 top-[calc(100%+8px)] z-[70] w-60 max-w-[calc(100vw-2rem)]">
          <div
            role="menu"
