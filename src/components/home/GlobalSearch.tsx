...
> = {
  brand: {
    ...
    chipIdle: 'bg-brand-50  text-brand-700  hover:bg-brand-100',
    disc: 'bg-brand-50  text-brand-600',
    pill: 'bg-brand-50  text-brand-700',
  },
  rose: {
    chipActive: 'bg-rose-600   text-white',
    chipIdle: 'bg-rose-50   text-rose-700   hover:bg-rose-100',
    disc: 'bg-rose-50   text-rose-600',
    pill: 'bg-rose-50   text-rose-700',
  },
  amber: {
    chipActive: 'bg-amber-500  text-white',
    chipIdle: 'bg-amber-50  text-amber-700  hover:bg-amber-100',
    disc: 'bg-amber-50  text-amber-600',
    pill: 'bg-amber-50  text-amber-700',
  },
  mint: {
    chipActive: 'bg-mint-500   text-white',
    chipIdle: 'bg-mint-50   text-mint-700   hover:bg-mint-100',
    disc: 'bg-mint-50   text-mint-600',
    pill: 'bg-mint-50   text-mint-700',
  },
  violet: {
    chipActive: 'bg-violet-600 text-white',
    chipIdle: 'bg-violet-50 text-violet-700 hover:bg-violet-100',
    disc: 'bg-violet-50 text-violet-600',
    pill: 'bg-violet-50 text-violet-700',
  },
  ink: {
    chipActive: 'bg-ink-900    text-white',
    chipIdle: 'bg-ink-100   text-ink-700    hover:bg-ink-200',
...
export function GlobalSearch({ initialQuery = '', variant = 'hero', className }: Props) {
  ...
  const flatHits: SearchHit[] = useMemo(() => data?.groups.flatMap((g) => g.hits) ?? [], [data]);

  useEffect(() => {
    setActive(-1);
  }, [data?.q, source, facets]);
  ...
  const setFacetValue = useCallback(
    (facetKey: string, value: string | undefined) => {
      if (source === 'all') return;
      setFacets((prev) => {
  ...
  return (
    <div ref={wrapRef} className={`${pillClasses} ${className ?? ''}`}>
      ...
      <form
        ...
        role="search"
        onSubmit={submitForm}
        className={`relative rounded-full bg-white ring-1 ring-ink-100 ${
          open
            ? 'shadow-[0_4px_14px_-4px_rgba(13,128,116,.18)]'
            : 'shadow-[0_18px_50px_-22px_rgba(11,53,48,.30)] focus-within:shadow-[0_22px_60px_-18px_rgba(13,128,116,.45)] hover:shadow-[0_22px_56px_-22px_rgba(11,53,48,.36)]'
        } flex items-center gap-2 py-1.5 pl-1.5 pr-3 transition-[box-shadow,ring] duration-300 focus-within:ring-2 focus-within:ring-brand-500 md:pr-5`}
      >
        <span
          className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-brand-50 text-brand-600 md:h-10 md:w-10"
          aria-hidden="true"
        >
          <Glyph glyph="search" className="h-4 w-4 md:h-[18px] md:w-[18px]" />
        </span>
        ...
        <input
          ...
          autoComplete="off"
          spellCheck={false}
          className="gs-search-input h-10 min-w-0 flex-1 appearance-none bg-transparent text-right text-[14px] text-ink-900 outline-none placeholder:text-ink-400 md:h-11 md:text-[15px]"
        />
        ...
        {q.length > 0 && (
          <button
            ...
            onClick={clearInput}
            aria-label="پاک کردن"
            className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-ink-400 transition-colors hover:bg-ink-50 hover:text-ink-700"
          >
            <Glyph glyph="x" className="h-4 w-4" />
          </button>
        )}

        {loading && (
          <span className="shrink-0 text-brand-500" aria-hidden="true">
            <Glyph glyph="spinner" className="h-4 w-4" />
          </span>
        )}
        ...
        <button
          type="submit"
          aria-label="جست‌وجو"
          className="inline-flex h-10 shrink-0 items-center justify-center gap-1.5 rounded-full bg-brand-500 px-4 text-[13px] font-extrabold text-white shadow-[0_8px_22px_-8px_rgba(13,128,116,.55)] transition-all duration-200 hover:scale-[1.02] hover:bg-brand-600 active:scale-[.98] md:h-11 md:px-5 md:text-[14px]"
        >
          <span className="hidden sm:inline">جست‌وجو</span>
          <Glyph glyph="search" className="h-4 w-4 sm:hidden" />
        </button>
      </form>
          ...
          <AnimatePresence>
            {open && geom && (
              <motion.div
                ...
                className="flex flex-col overflow-hidden rounded-3xl bg-white shadow-[0_30px_80px_-20px_rgba(11,53,48,.45)] ring-1 ring-ink-100"
                role="region"
                aria-label="پیشنهادهای جست‌وجو"
                dir="rtl"
              >
                {/* Source chips */}
                <div className="flex shrink-0 flex-wrap items-center gap-1.5 border-b border-ink-50 px-3 pb-2 pt-3">
                  <SourceChip
                    label="همه"
                    glyph="grid"
                    accent="ink"
                ...
                {source !== 'all' && FACETS[source]?.length > 0 && (
                  <div className="flex shrink-0 flex-wrap items-center gap-1.5 border-b border-ink-50 bg-ink-50/40 px-3 pb-2 pt-2">
                    <span className="inline-flex h-6 items-center gap-1 text-[11px] font-extrabold uppercase tracking-wider text-ink-500">
                      <Glyph glyph="sliders" className="h-3 w-3" />
                      فیلتر
                    </span>
                      ...
                      return group.chips.map((chip) => {
                        ...
                        return (
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              setFacetValue(group.key, isActive ? undefined : chip.value)
                            }
                            className={`inline-flex h-7 items-center gap-1 rounded-full px-2.5 text-[11.5px] font-extrabold transition-all ${
                              isActive
                    ...
                    {activeFacetKeys.length > 0 && (
                      <button
                        ...
                        className="mr-auto inline-flex h-7 items-center gap-1 px-2 text-[11px] font-bold text-ink-500 transition-colors hover:text-rose-600"
                      >
                        پاک کردن فیلترها
                      </button>
                    )}
                ...
                <div
                  ...
                  className="custom-scroll min-h-0 flex-1 overflow-y-auto"
                >
                  {q.trim().length < 2 ? (
                    <EmptyInputBody
                      recents={recents}
                ...
                {/* Footer */}
                <div className="flex shrink-0 items-center justify-between gap-3 border-t border-ink-50 bg-ink-50/50 px-4 py-2.5 text-[11.5px] text-ink-500">
                  ...
                  {q.trim().length >= 2 && (
                    <button
                      type="button"
                      onClick={() => submitForm()}
                      className="inline-flex items-center gap-1.5 font-extrabold text-brand-700 transition-colors hover:text-brand-800"
                    >
                      مشاهده همه‌ی نتایج برای «{q.trim()}»
                      <Glyph glyph="arrow-left" className="h-3.5 w-3.5" />
                    </button>
                  )}
                </div>
              </motion.div>
...
function SourceChip({
  ...
  return (
    <button
      ...
      onClick={onClick}
      aria-pressed={active}
      className={`inline-flex h-8 items-center gap-1.5 rounded-full px-3 text-[12px] font-extrabold tabular-nums transition-all duration-150 ${active ? `${a.chipActive} shadow-[0_4px_14px_-4px_rgba(0,0,0,.20)]` : a.chipIdle}`}
    >
      <Glyph glyph={glyph} className="h-3.5 w-3.5" />
      <span>{label}</span>
      {count > 0 && (
        <span
          className={`inline-flex h-[20px] min-w-[20px] items-center justify-center rounded-full px-1 text-[10.5px] font-extrabold ${active ? 'bg-white/25 text-white' : 'bg-white/70 text-ink-700'}`}
        >
  ...
  return (
    <span className="inline-flex items-center gap-0.5">
      {items.map((k, i) => (
        <kbd
          key={`${k}-${i}`}
          className="inline-flex h-5 min-w-[20px] items-center justify-center rounded bg-white px-1 font-sans text-[10.5px] font-extrabold text-ink-700 ring-1 ring-ink-200"
        >
          {k}
        </kbd>
      ))}
    ...
    <div className="p-4">
      <div className="mb-4">
        <div className="mb-2.5 flex items-center gap-2 text-ink-500">
          <Glyph glyph="flame" className="h-3.5 w-3.5 text-amber-500" />
          ...
          {TRENDING_QUERIES.map((t) => {
            ...
            return (
              <button
                ...
                className={`inline-flex h-8 items-center gap-1.5 rounded-full px-3 ${ACCENT[accent].chipIdle} text-[12px] font-bold transition-all`}
              >
                {t.source && <Glyph glyph={SEARCH_SOURCES[t.source].glyph} className="h-3 w-3" />}
                {t.label}
              </button>
            );
          })}
      ...
      {recents.length > 0 ? (
        <div>
          <div className="mb-2.5 flex items-center justify-between">
            <div className="flex items-center gap-2 text-ink-500">
              <Glyph glyph="clock" className="h-3.5 w-3.5" />
              <span className="text-[11.5px] font-extrabold uppercase tracking-wider">
                جست‌وجوهای اخیر
              </span>
            </div>
            <button
              ...
              className="text-[11px] font-bold text-ink-400 hover:text-rose-500"
            >
              پاک کردن همه
            </button>
          </div>
          <div className="flex flex-col gap-0.5">
            {recents.map((r) => (
              <button
                ...
                className="group flex w-full items-center gap-3 rounded-xl px-2.5 py-2 text-right transition-colors hover:bg-ink-50"
              >
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-ink-50 text-ink-500">
                  <Glyph glyph="clock" className="h-3.5 w-3.5" />
                </span>
                <span className="flex-1 truncate text-[13px] font-bold text-ink-800">{r}</span>
        ...
        <p className="py-3 text-center text-[12.5px] leading-7 text-ink-400">
          عبارت موردنظر را تایپ کنید — می‌توانید بخشی از عنوان کمپین، اسم مددکار، نام مدرس، تیتر
          روایت یا حتی بخشی از نام مجرم را جست‌وجو کنید.
        </p>
      )}
    </div>
  );
  ...
  return (
    <div className="space-y-2.5 p-4" aria-live="polite" aria-busy="true">
      {[0, 1, 2, 3].map((i) => (
        <div
          key={i}
          className="flex animate-pulse items-center gap-3 rounded-xl bg-ink-50/60 p-2.5"
        >
          <div className="h-11 w-11 rounded-xl bg-ink-100" />
          <div className="min-w-0 flex-1 space-y-1.5">
            <div className="h-3 w-[60%] rounded-full bg-ink-100" />
            <div className="h-2.5 w-[40%] rounded-full bg-ink-100" />
          </div>
  ...
  return (
    <div className="p-6 text-center" role="alert">
      ...
      <p className="mt-1 text-[12px] text-ink-500">
        دوباره تلاش کنید یا عبارت دیگری را امتحان کنید.
      </p>
    </div>
  );
}
...
function NoResultsBody({ q }: { q: string }) {
  return (
    <div className="p-6 text-center" aria-live="polite">
      <div className="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-brand-50 to-brand-100 text-brand-600">
        <Glyph glyph="search" className="h-6 w-6" />
      </div>
      <p className="text-[14px] font-extrabold text-ink-900">نتیجه‌ای برای «{q.trim()}» پیدا نشد</p>
      <p className="mt-1 text-[12px] leading-7 text-ink-500">
  ...
  return (
    <div className="py-2">
          ...
          <div key={g.source} className="mb-1.5 last:mb-0">
            <div className="flex items-center justify-between px-4 pb-1 pt-2.5">
              <div className="flex items-center gap-2">
                <span
                  className={`inline-flex h-6 w-6 items-center justify-center rounded-lg ${a.disc}`}
                >
                  <Glyph glyph={meta.glyph} className="h-3.5 w-3.5" />
                </span>
                ...
                return (
                  ...
                  >
                    <button
                      type="button"
                      onClick={() => onPick(h)}
                      className={`flex w-full items-center gap-3 rounded-xl px-2.5 py-2 text-right transition-colors ${isActive ? 'bg-brand-50' : 'hover:bg-ink-50'}`}
                    >
                      <span className="relative h-11 w-11 shrink-0 overflow-hidden rounded-xl bg-ink-100">
                        ...
                        <span
                          className={`absolute inset-0 flex items-center justify-center ${a.disc}`}
                        >
                          <Glyph glyph={meta.glyph} className="h-5 w-5" />
                        </span>
                        {h.thumb && (
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        {(h.subtitle || h.badge) && (
                          <span className="mt-0.5 block truncate text-[11.5px] font-medium text-ink-500">
                            {h.subtitle}
                            {h.subtitle && h.badge && (
                              <span className="mx-1.5 text-ink-300">·</span>
                            )}
                            {h.badge && <span className="font-bold text-ink-700">{h.badge}</span>}
                          </span>
                        )}
                      </span>
                      {h.pill && (
                        <span
                          className={`inline-flex h-7 shrink-0 items-center rounded-full px-2.5 text-[11px] font-extrabold ${a.pill}`}
                        >
                          {h.pill}
                        </span>
                      )}
                      <span
                        className={`shrink-0 transition-all ${isActive ? '-translate-x-0.5 text-brand-600' : 'text-ink-300'}`}
                      >
                        <Glyph glyph="arrow-left" className="h-4 w-4" />
                      </span>
                    </button>
                  </li>
                );
      ...
      {data.errored.length > 0 && (
        <p className="px-4 pb-3 pt-2 text-center text-[11px] text-ink-400">
