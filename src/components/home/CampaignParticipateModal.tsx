    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          className="fixed inset-0 z-[120] flex items-end justify-center bg-ink-900/80 p-0 backdrop-blur-md sm:items-center sm:p-5"
          onClick={(e) => {
        ...
        >
          <motion.div
            ...
            className="relative flex max-h-[94vh] w-full max-w-[640px] flex-col overflow-hidden rounded-t-[28px] bg-white shadow-[0_50px_100px_-25px_rgba(0,0,0,.55)] sm:rounded-[28px]"
          >
            {/* ── Hero header (cover + title + sponsor + close) ───── */}
            <div className="relative h-[180px] shrink-0 overflow-hidden bg-ink-800 sm:h-[200px]">
              <SmartImage
              ...
              <button
                ...
                aria-label="بستن"
                className="absolute left-3 top-3 z-[2] flex h-10 w-10 items-center justify-center rounded-full bg-black/45 text-white ring-1 ring-white/20 backdrop-blur transition-all duration-200 hover:scale-105 hover:bg-rose-500 active:scale-95"
              >
                <svg
                  width="16"
                  height="16"
              ...
              {deadlineRemaining && !deadlineRemaining.expired && (
                <div className="absolute right-3 top-3 z-[2] inline-flex h-9 items-center gap-1.5 rounded-full bg-black/45 px-3 text-[12px] font-extrabold text-white ring-1 ring-white/20 backdrop-blur">
                  <svg
                    width="14"
                    height="14"
                    viewBox="0 0 24 24"
              ...
              <div className="absolute inset-x-0 bottom-0 z-[1] p-4 sm:p-5">
                <h2 className="line-clamp-2 text-[16px] font-extrabold leading-7 text-white sm:text-[18px]">
                  {campaign.title}
                </h2>
                <p className="mt-1 inline-flex items-center gap-1.5 text-[12.5px] font-medium text-white/80">
                  <svg
                    width="14"
                    height="14"
                    viewBox="0 0 24 24"
            ...
            <div className="flex-1 space-y-5 overflow-y-auto px-4 py-5 sm:px-6">
              {/* Progress meter */}
              <div>
                <div className="mb-2 flex items-center justify-between text-[12.5px] font-bold text-ink-600">
                  <span>
                ...
                <div className="h-2.5 overflow-hidden rounded-full bg-ink-100">
                  <div
                    className="h-full rounded-full bg-gradient-to-l from-mint-500 to-brand-600 transition-[width] duration-700 ease-out"
                    style={{ width: `${progressPct}%` }}
                  />
                </div>
                <p className="mt-2 text-[11.5px] font-medium tabular-nums text-ink-500">
                  {remaining > 0 ? (
                    <>
                      سهم باقی‌مانده برای خرید:{' '}
                      <strong className="text-ink-800">{formatPersianNumber(remaining)}</strong>
                    </>
                  ) : (
              ...
              {!isClosed && (
                <div className="rounded-2xl border border-ink-100 bg-gradient-to-b from-white to-brand-50/30 p-4 sm:p-5">
                  ...
                  <div className="mt-4 flex flex-wrap items-center gap-1.5">
                    <span className="ml-1 text-[11.5px] font-bold text-ink-500">انتخاب سریع:</span>
                    {quickPicks.map((v, i) => {
                ...
                >
                  ...
                  <div className="mt-2 flex items-baseline justify-between">
                    ...
                    <div className="flex flex-col items-end">
                      <span className="text-[24px] font-extrabold tabular-nums leading-none sm:text-[28px]">
                        {formatPersianNumber(totalToman)}
                      </span>
                      <span className="mt-0.5 text-[11px] opacity-80">تومان</span>
                    </div>
                  </div>
                  <div className="mt-1 flex items-center justify-end gap-1 text-[11.5px] tabular-nums opacity-75">
                    ...
                    <div className="mt-3 inline-flex items-center gap-1.5 border-t border-white/15 pt-3 text-[11.5px] font-medium opacity-90">
                      ...
                      <strong className="font-extrabold">
                        {shareOfCampaignPct < 0.1
                          ...
                          : `٪${formatPersianNumber(shareOfCampaignPct.toFixed(shareOfCampaignPct < 1 ? 2 : 1))}`}
                      </strong>
                ...
                >
                  ...
                  <div className="grid grid-cols-1 gap-3 px-4 pb-4 sm:grid-cols-2">
                    ...
                    <p className="text-[11px] font-medium leading-6 text-ink-500 sm:col-span-2">
                      این اطلاعات اختیاری‌اند و در صورت ارسال، برای ارتباط درگاه با شما استفاده
                      می‌شوند. در غیر این صورت، از اطلاعات پروفایل شما استفاده خواهد شد.
                    </p>
                  </div>
                </details>
              )}
              ...
              <div className="flex items-start gap-3 rounded-2xl border border-ink-100 bg-ink-50 p-3.5">
                ...
                <p className="text-[11.5px] font-medium leading-6 text-ink-600">
                  پرداخت از طریق درگاه رسمی و امن انجام می‌شود. پس از انتخاب، تا{' '}
                  <strong className="text-ink-900">
                    {formatPersianNumber(RESERVE_MINUTES)} دقیقه
                  </strong>
              ...
              <AnimatePresence>
                {error && (
                  <motion.div
                    ...
                    exit={{ opacity: 0 }}
                    role="alert"
                    className="flex items-start gap-2 rounded-xl bg-rose-50 p-3 text-[12.5px] font-bold text-rose-700 ring-1 ring-rose-200"
                  >
                    <svg
                      width="16"
                      height="16"
              ...
              {isClosed && (
                <div className="flex items-start gap-2 rounded-xl bg-amber-50 p-3 text-[12.5px] font-bold text-amber-800 ring-1 ring-amber-200">
                  <svg
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
            ...
            <div className="flex shrink-0 items-center justify-between gap-3 border-t border-ink-100 bg-white px-4 py-3.5 sm:px-6">
              <button
                ...
                disabled={submitting}
                className="h-12 rounded-full bg-ink-50 px-5 text-[13px] font-extrabold text-ink-700 transition-colors hover:bg-ink-100 disabled:opacity-50"
              >
                انصراف
              </button>
              <button
                ...
                onClick={onSubmit}
                disabled={!canSubmit}
                className="relative inline-flex h-12 flex-1 items-center justify-center gap-2 rounded-full bg-gradient-to-l from-mint-500 to-brand-700 text-[14px] font-extrabold text-white shadow-[0_10px_24px_-6px_rgba(13,128,116,.55)] transition-all duration-150 hover:brightness-105 active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-60"
              >
                {submitting ? (
                  <>
                    <svg
