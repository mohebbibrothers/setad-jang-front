              ...
              >
                <div className="relative aspect-[16/10] bg-brand-50">
                  {first?.media_type === 'image' && first.url ? (
                    ...
                    <img
                      src={thumb}
                      alt={item.title || 'ویدئوی تبیین'}
                      className="h-full w-full object-cover"
                    />
                  ) : (
                    <div className="flex h-full items-center justify-center bg-gradient-to-br from-brand-500 to-brand-800 font-black text-white">
                      جهاد تبیین
                    </div>
                  )}
                </div>
                <div className="p-4">
                  ...
                  {item.author_username && (
                    <p className="mt-2 text-xs font-bold text-ink-500">{item.author_username}</p>
                  )}
                </div>
              </Link>
            );
